...
try {
    ...
    try {
        ...
        # Start-Process ולא הפעלה רגילה: כשמריצים python ישירות מ-PowerShell,
        # בין אם בצינור ובין אם עם *> לקובץ, PowerShell עומד באמצע ולא מרוקן
        # את הפלט. מאגר הפלט מתמלא, print() נחסם, והבנייה קופאת לנצח — אפס
        # CPU, אפס I/O, בלי שורה אחת ביומן. כאן מערכת ההפעלה כותבת ישירות
        # לקובץ ו-PowerShell לא נוגע בזרם בכלל
        $stamp = '{0:yyyy-MM-dd-HHmm}' -f (Get-Date)
        ...
        $buildCode = $proc.ExitCode
        foreach ($f in @($buildLog, $buildErr)) {
